...
{
  if (argc != 3) {
	  std::cout << "La llamada correcta es: ./flip_host {horizontal | vertical | rot90derecha} img_salida.jpg." << std::endl;
	  return -1;
  }
  ...
  //5.
	...
	if (strcmp(argv[1],"horizontal")==0){
		...
		for (int fila = 0; fila < filas; fila++){
			for (int col = 0; col < columnas; col++){
				for (int color = 0; color < spectrum; color++){
					pixeles_entrada[indice_p] = img(fila,col,0,color);
					indice_p++;
				}
			}
		}
		...
		if(strcmp(argv[1],"vertical")==0){
			...
			std::cout << "MODO ROT_90_DERECHA" << std::endl;
		}
		for (int col = 0; col < columnas; col++){
			for (int fila = 0; fila < filas; fila++){
				for (int color = 0; color < spectrum; color++){
...
//printf("9.Kernel lanzado\n");
  ...
  cimg_library::CImg<unsigned char> salida(filas,columnas,1,spectrum,0);
  indice_p = 0; //índice para recorrer el vector "pixeles_entrada"
  if (strcmp(argv[1],"vertical")==0){
		for (int col = 0; col < columnas; col++){
	      for (int fila = 0; fila < filas; fila++){
		    for (int color = 0; color < spectrum; color++){
			  salida(fila,col,0,color) = pixeles_salida[indice_p];
			  indice_p++;
		    }
		  }
		}
