...
void procesoThread(Cargador& cargador, unsigned int indiceBase, unsigned int cantidadACargar, unsigned char* buffer, 
                    cl_context context, cl_command_queue command_queue, cl_kernel kernel){
  ...
  int err;
  unsigned long long int tiempoKernel;
  unsigned long long int tiempoHostDev;
  ...
  while (numCargas > 0){
    ...
    for(int i = 0; i < numCargas; ++i){
        ...
        err = clEnqueueWriteBuffer(command_queue, in_out_device_object, CL_TRUE, i * sizeof(unsigned char) * cargador.num_elementos_imagen, sizeof(unsigned char) * cargador.num_elementos_imagen, buffer, 0, NULL, &host_dev_event);
        ...
        err = clSetKernelArg(kernel, 4, sizeof(unsigned int), &cargador.num_elementos);
        cl_error(err, "Failed to set argument 4\n");
        //Specify number of WI
        size_t global_size;
        if (strcmp(cargador.modo,"horizontal")==0){
  ...
  std::cout << "Tiempo de ejecución de kernel total: " << tiempoKernel << " nanoseconds." << std::endl;
  std::cout << "Numero de imagenes procesadas: " << numImagenes << "." << std::endl;
  int swaps_por_wi = 12;
  std::cout << "Bandwidth del kernel: " << ((float)(sizeof(unsigned char) * global_size * swaps_por_wi * numImagenes)) / tiempoKernel << " Bytes/nanosecond." << std::endl;
  std::cout << "Bandwith of the Host->Device: " << static_cast<float>(sizeof(unsigned char) * cargador.num_elementos_imagen * numImagenes) / tiempoHostDev << " Bytes/nanosecond." << std::endl;
  std::cout << "Bandwith of the Device->Host: " << static_cast<float>(sizeof(unsigned char) * cargador.num_elementos_imagen * numImagenes) / tiempoDevHost << " Bytes/nanosecond." << std::endl;
...
int main(int argc, char** argv)
{
  ...
  unsigned char* pixeles_entrada = new unsigned char[100];//[num_elementos * spectrum * num_imagenes];
	...
	std::cout << "Se han cargado las imagenes" << std::endl;
	unsigned char* pixeles_salida = new unsigned char[100];//[num_elementos * spectrum * num_imagenes];
  ...
  cl_mem in_out_device_object_1  = clCreateBuffer(context1, CL_MEM_READ_WRITE, sizeof(unsigned char) * num_elementos * spectrum * num_imagenes, NULL, &err);
  std::cout << "Buffer 2" << std::endl;
  cl_mem in_out_device_object_2  = clCreateBuffer(context2, CL_MEM_READ_WRITE, sizeof(unsigned char) * num_elementos * spectrum * num_imagenes, NULL, &err);
  ...
  err = clEnqueueWriteBuffer(command_queue1, in_out_device_object_1, CL_TRUE, 0, sizeof(unsigned char) * num_elementos * spectrum * num_imagenes_input, pixeles_entrada, 0, NULL, &host_dev_event1);
  cl_error(err, "Failed to enqueue a write command\n");
    // Write date into the memory object 
  cl_event host_dev_event2; //medir tiempo de paso de memoria HOST->DEVICE
  std::cout << "Cargo al buffer 2" << std::endl;
  err = clEnqueueWriteBuffer(command_queue2, in_out_device_object_2, CL_TRUE, 0, sizeof(unsigned char) * num_elementos * spectrum * num_imagenes_input, pixeles_entrada, 0, NULL, &host_dev_event2);
  cl_error(err, "Failed to enqueue a write command\n");
  ...
  cl_error(err, "Failed to set argument 4\n");
  unsigned int num_wi_por_imagen = 0;
  if (strcmp(argv[3],"horizontal")==0){
	num_wi_por_imagen = (columnas/2) * filas;
	global_size = (columnas/2) * filas * num_imagenes_input;
  } else {
	num_wi_por_imagen = (filas/2) * columnas;
	global_size = (filas/2) * columnas * num_imagenes_input;
  }
  err = clSetKernelArg(kernel1, 5, sizeof(unsigned int), &num_wi_por_imagen);
  cl_error(err, "Failed to set argument 5\n");
  err = clSetKernelArg(kernel1, 6, sizeof(unsigned int), &num_imagenes_input);
  cl_error(err, "Failed to set argument 6\n");
  err = clSetKernelArg(kernel2, 5, sizeof(unsigned int), &num_wi_por_imagen);
  cl_error(err, "Failed to set argument 5\n");
  err = clSetKernelArg(kernel2, 6, sizeof(unsigned int), &num_imagenes_input);
  cl_error(err, "Failed to set argument 6\n");
