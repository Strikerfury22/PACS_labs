...
{
  if (argc != 5) {
	  std::cout << "La llamada correcta es: ./flip_host tamano_buffer img_entrada.jpg {horizontal | vertical} num_imagenes " << std::endl;
	  return -1;
  }
  ...
  const int tamanoBuffer = std::atoi(argv[1]);
  ...
  int indices[2] = {0, cargaKernels[0]};
